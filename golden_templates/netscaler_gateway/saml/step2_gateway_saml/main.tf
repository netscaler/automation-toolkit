...
# scp "okta.cert" nsroot@10.0.10.10:/nsconfig/ssl/
resource "citrixadc_systemfile" "okta_certificate" {
  filename     = var.okta_certfile_name
  filelocation = "/nsconfig/ssl/"
  filecontent  = file("${path.module}/${var.okta_certfile_name}")
}
...
# add ssl certKey "OKTA-CA" -cert okta.cert
resource "citrixadc_sslcertkey" "okta_certkey" {
  certkey = "OKTA-CA"
  cert    = "okta.cert"
  depends_on = [
    citrixadc_systemfile.okta_certificate
  ]
}
...
# unbind audit syslogGlobal SETSYSLOGPARAMS_ADV_POL 
...
# add authentication samlAction OKTA -samlIdPCertName "OKTA CA" -samlSigningCertName remote.mycoolcompany.com -samlRedirectUrl "https://trial-8385130.okta.com/app/citrixnetscalergateway_saml/exk4aksdlp46lHubx697/sso/saml" -samlUserField NameID -samlIssuerName "http://www.okta.com/exk4aksdlp46lHubx697" -logoutURL "https://trial-8385130.okta.com"
resource "citrixadc_authenticationsamlaction" "authenticationsamlaction_uoxqpdyvao" {
  name                = "OKTA"
  samlidpcertname     = citrixadc_sslcertkey.okta_certkey.certkey  #"OKTA-CA"
  samlsigningcertname = citrixadc_sslcertkey.tf_sslcertkey.certkey #"remote.mycoolcompany.com"
  samlredirecturl     = var.authenticationsamlaction_samlredirecturl
  samluserfield       = var.authenticationsamlaction_samluserfield
  samlissuername      = var.authenticationsamlaction_samlissuername
  logouturl           = var.authenticationsamlaction_logouturl
}

# add authentication Policy Gateway_SAML_Policy -rule true -action OKTA
resource "citrixadc_authenticationpolicy" "authenticationsamlpolicy_olpvjchvkq" {
  name   = "Gateway_SAML_Policy"
  rule   = "true"
  action = citrixadc_authenticationsamlaction.authenticationsamlaction_uoxqpdyvao.name #"OKTA"
}

# bind authentication vserver Gateway_Auth_vServer -policy Gateway_SAML_Policy -priority 100 -gotoPriorityExpression NEXT
resource "citrixadc_authenticationvserver_authenticationpolicy_binding" "tf_bind" {
  ...
  policy                 = citrixadc_authenticationpolicy.authenticationsamlpolicy_olpvjchvkq.name
  priority               = 100
  gotopriorityexpression = "NEXT"
}
