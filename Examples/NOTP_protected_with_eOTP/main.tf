...
resource "citrixadc_authenticationemailaction" "mailact" {
  name = "Auth_Act_email_${var.suffix}"
  username = var.email_sender
  password = var.email_password
  serverurl = "${var.email_server_protocol}://${var.email_server_ip_or_name}:${var.email_server_port}"
...
resource "citrixadc_authenticationpolicy" "mailpol" {
  name = "Auth_Pol_eOTP_${var.suffix}"
  rule = "TRUE"
  action = citrixadc_authenticationemailaction.mailact.name
}
...
resource "citrixadc_authenticationpolicylabel" "pollabeotp" {
  labelname = "Auth_Lab_eOTP_${var.suffix}"
  loginschema = "LSCHEMA_INT"
}
...
resource "citrixadc_authenticationpolicylabel_authenticationpolicy_binding" "bindlabldap" {
  ...
  priority = 100
  gotopriorityexpression = "NEXT"
  nextfactor = citrixadc_authenticationpolicylabel.pollabeotp.labelname
}

resource "citrixadc_authenticationpolicylabel_authenticationpolicy_binding" "bindlabeotp" {
  labelname = citrixadc_authenticationpolicylabel.pollabeotp.labelname
  policyname = citrixadc_authenticationpolicy.mailpol.name
  priority = 100
  gotopriorityexpression = "NEXT"
